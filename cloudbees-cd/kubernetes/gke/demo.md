...
1. To download the CLoudBees CD/RO demo values file, run:
    ```bash
    curl -fsSL -o cloudbees-cd-demo.yaml $DEMO_FILE_URL
    ```
1. To install CloudBees CD/RO from the `cloudbees/cd` Helm repo, run:
    ```bash
...
1. To get CloudBees CD/RO URL, run:
   ...
   ```
1. To get the autogenerated password for `admin`, run:
    ```bash
    kubectl get secret $HELM_RELEASE-cloudbees-flow-credentials \
    --namespace $NAMESPACE \
    -o jsonpath="{.data.CBF_SERVER_ADMIN_PASSWORD}" | base64 --decode; echo
    ```
1. Open your supported browser and paste the URL returned by the previous command.
1. To log in to the instance for the username, enter `admin`, and for the password, enter the autogenerated password returned from the previous command.  
...
1. To delete the CloudBees CD/RO installation and `namespace`, run: 
   ```bash
    ...
    ```
1. To delete the GKE cluster, run:
   ```bash
