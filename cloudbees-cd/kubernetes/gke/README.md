...
- Set environment variables  
  ...
  # 3 or more is recommended for production
  ...
  # n1-standard-8 or large is recommended for production
  GKE_CLUSTER_MACHINE_TYPE=<gke-cluster-machine-type>
  HELM_RELEASE=<cd-helm-release>
  NAMESPACE=<cd-k8s-namespace>
  ```  
- Create GKE cluster
    ```bash
...
- Create DNS record for the SQL instance
    ...
    gcloud dns record-sets transaction add $GCP_DB_IP \
      --name=$GCP_DB_INSTANCE_NAME \
      --ttl=300 \
      --type=A \
      --zone=$GCP_DNS_ZONE
